{{!--
  ...
  {{< layouts/default }}
    {{$classnames}}podcast-home{{/classnames}}
    ...
    {{$meta}}
      {{#page}}
        ...
        <meta name="page-tracking" content="sho:sports:podcasts:{{meta.valueMap.navigationPageName}}" />
        <meta property="og:image" content="https://www.sho.com/site/image-bin/images/0_0_0/0_0_0_prm-ogsports_1280x640.jpg" />
				...
				<link type="application/rss+xml" rel="alternate" title="{{meta.valueMap.navigationPageName}}" {{#subscriptionsTileGroupView.tileList}} 
				{{#iter.isLast}} href="{{ctaLink}}" {{/iter.isLast}} {{/subscriptionsTileGroupView.tileList}}>
      {{/page}}
    {{/meta}}
    ...
    {{$content}}
      {{#page}} 
				...
				<section class="hero hero--no-accent {{#primaryHero.heroView.panelLink}}hero--panel-link{{/primaryHero.heroView.panelLink}}" data-context="hero">  
					{{#primaryHero}}         
					...
					<section class="section section--large section--inner">
						<div class="subscription-list" data-track data-context="podcasts:subscribe">
							...
							{{#tileList}}
								<a href="{{ctaLink}}" class="subscription-list__item--img subscription-list__item--feed-{{subTitle}} button--solid-dark-blue" data-label="{{ctaLabel}}" style="background-image:url({{smallImageUrl}})"></a>
							{{/tileList}}
							</div>
						</section>  
					{{/subscriptionsTileGroupView}}	
            ...
            <section class="section section--large section--inner">
              ...
              {{#tileList}}	
                <div class="podcast-list__item data-track" data-context="podcasts:episodes">
                  <!-- <div class="podcast-list__item__field--play">
                    <a href ="link" class="icon icon--large icon--play" data-label="{{title}}"></a>
                  </div> -->
                  <div class="podcast-list__item__field--title">
                    <a {{#ctaLink}}href="{{ctaLink}}"{{/ctaLink}}  class="podcast-list__item__field--link" data-label="{{title}}">{{{title}}}</a>
                  </div>
                  <div class="podcast-list__item__field--date">{{{topLine}}}<span class="podcast-list__item--separator">| </span></div>
