...
/* check if global report suite ID variable already exists, otherwise default to production ID */
if (!omniture_rsid) {
    var omniture_rsid = "cbssho,cbsshoglobal";
}

var s_account=omniture_rsid;  // production: "cbssho,cbsshoglobal" dev: "cbsshodev,cbsshoglobaldev"
var s=s_gi(s_account)         // this instantiates the 's' superglobal, aka creates the tracker
...
/* E-commerce Config */
s.currencyCode="USD"
s.trackingServer="metrics.sho.com";
s.trackingServerSecure="smetrics.sho.com";
/* Link Tracking Config */
s.trackDownloadLinks=true
s.trackExternalLinks=true
s.trackInlineStats=true
s.linkDownloadFileTypes="exe,zip,wav,mp3,mov,mpg,avi,wmv,doc,pdf,xls,m4v,xml,rss,mrss,3gp"
s.linkInternalFilters="javascript:,sho.com,localhost:8080"
s.useForcedLinkTracking=false
s.linkLeaveQueryString=false
s.linkTrackVars="None"
s.linkTrackEvents="None"
s.visitor = Visitor.getInstance("FD4702BE533343690A490D4C@AdobeOrg");
/*  Capture Omniture Visitor ID  */
s.eVar19 = s.visitor.getMarketingCloudVisitorID();
...
function s_doPlugins(s) {
    ...
    /* check for two different possible campaign params
    *  with s_cid taking preference over source if both are found */
	  s.campaign = s.Util.getQueryParam('s_cid').split('#')[0];

    if(s.campaign == '') {
        s.campaign = s.Util.getQueryParam('source');
    }
    /* getValOnce plugin (via cookie) ensures page refresh
    * or back button use does not cause overcounting */
    s.campaign = s.getValOnce(s.campaign,'gvo_campaign',30);

    /* for internal campaigns */
    s.eVar52 = s.Util.getQueryParam('i_cid').split('#')[0];
    s.eVar52 = s.getValOnce(s.eVar52,'gvo_eVar52',0);

    /* for external campaigns */
    s.eVar53 = s.campaign;
    s.eVar53 = s.getValOnce(s.eVar53,'gvo_eVar53',30);

    /* Optimizely Adobe Analytics SiteCatalyst Integration */
    window.optimizely = window.optimizely || [];
    window.optimizely.push("activateSiteCatalyst");

    //start ECID monitoring code
    ...
    if (typeof(Visitor) != "undefined") {
      ...
      else { s.contextData['mcid.icsmcvid'] = "NA"; }
      ...
      else { s.contextData['mcid.mcidcto'] = "NA"; }
      ...
      else { s.contextData['mcid.aidcto'] = "NA"; }
      ...
      else { s.contextData['mcid.aamidcto'] = "NA"; }
...
/* Plugin: getPreviousValue 1.1 (added 03/2018 MLemay) */
