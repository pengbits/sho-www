...
class Slider {
  ...
  constructor(cfg) {
    var el = $(cfg.el); Object.assign(this, {
      ...
      'gridAt':     el.data('gridAt') ? parseInt(el.data('gridAt')) : null, // undo the slider behaviors after a certain width is reached (ie no slider on desktop)
      'gridUntil':  el.data('gridUntil') ? parseInt(el.data('gridUntil')) : null, // limit the slider behaviors to the point after a certain width is reached (ie no slider on mobile)
   ...
   */
  validateLayout() {
    this.log('validating layout display='+this.getDisplay());
    this.display = this.getDisplay()
  }

  getDisplay(){
    if(this.gridAt){
      return (this.gridAt < this.windowWidth) ? 'grid' : 'inline';
    }
    else if(this.gridUntil){
      return (this.windowWidth < this.gridUntil) ? 'grid' : 'inline';
    }
    else return 'inline'
  }
  ...
  validateControls() {
    ...
    if((this.containerWidth > this.contentWidth) ||
       (this.display == 'grid') ||
       (isTouchDevice() && this.windowWidth < this.controlBreakpoint)) {
