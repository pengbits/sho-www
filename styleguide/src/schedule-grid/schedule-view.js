...
import { isDST, convertTZ } from '../utils/date';
...
export default class ScheduleView {
  ...
  increaseHeight(model){
    ...
    const { startDateTime, endDateTime } = model;

    // height needs to be adjusted if it's a changeover DST day
    if (isDST(startDateTime) && !isDST(endDateTime) && !this.hasDSTChanged) {
      this.height += 10416; // to accomodate extra hour
      this.hasDSTChanged = true;
    } else if (!isDST(startDateTime) && isDST(endDateTime) && !this.hasDSTChanged) {
  ...
  updateScale(model){
    ...
    // hour axis 
    this.axis = d3.svg.axis().scale(this.yScale).orient("left").ticks(d3.time.hour, 1).tickFormat(this.formatTimeForYAxis);
  ...
  formatTimeForYAxis(raw) {
    const hrs = raw.getHours();
    const hr = hrs > 12 ? hrs - 12 : hrs == 0 ? 12 : hrs;
    const m = hrs >= 12 ? 'p' : 'a';
    return hr + m;
  }
  ...
  timeZoneOffset(localTime){
    ...
    // convert local to ET
    var ET = convertTZ(localTime);
    // difference in milliseconds of ET to UTC
    var easternTimeZoneOffset = isDST(ET) ? 14400000 : 18000000; // different offset if we are in daylight savings time  
